...
const KPICard = ({
    ...
    >
        <div className="flex justify-between items-start">
            <div className="p-3 rounded-full bg-gray-100 dark:bg-brand-dark">
                {icon}
            </div>
        ...
        <Link
            to={link}
            className="text-sm font-semibold text-brand-gold hover:underline mt-4 flex items-center"
        >
            {linkText}
...
const QuickActionCard = ({ icon, title, value, link }) => (
    <Link
        to={link}
        className="bg-gray-50 dark:bg-brand-dark p-4 rounded-lg flex items-center gap-4 hover:bg-gray-100 dark:hover:bg-brand-dark-200 transition-colors"
    >
...
const DashboardPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [overviewRes, healthRes, donationsRes] =
                    await Promise.all([
                        adminAPI.dashboard.getOverview(timeRange),
                        adminAPI.dashboard.getSystemHealth(),

                        // Get completed donations for real donation statistics
                        adminAPI.donations.getDonations({
                            status: 'Completed',
                            page: 1,
                            limit: 50
                        })
    ...
    if (!dashboardData) {
        return (
            <div className="text-center p-8 bg-red-100 dark:bg-red-900/50 text-red-700 rounded-lg">
    ...
    const donationSummary =
        donationData?.summary ||
        donationData?.data?.summary ||
        {};

    const totalDonationAmount =
        Number(
            donationSummary.completedAmount ??
            donationSummary.totalAmount ??
            0
        ) || 0;
    ...
    const donationsInRange = rawDonations.filter((donation: any) => {
        const donationDate = new Date(
            donation.donationDate ||
            donation.createdAt
        );

        return donationDate >= rangeStartDate;
    });
    ...
    donationsInRange.forEach((donation: any) => {
        const date = new Date(
            donation.donationDate ||
            donation.createdAt
        );
        ...
        if (date.getFullYear() === currentYear) {
            monthlyDonations[date.getMonth()] +=
    ...
    const registrationTrend =
        dashboardData?.charts?.registrationTrend || [];
    ...
    registrationTrend.forEach((item: any) => {
        if (
            ...
            monthlyUsers[item._id.month - 1] =
                Number(item.users) || 0;
        }
    });

    // Only show relevant months for current year.
    const currentMonth = new Date().getMonth();

    const chartLabels = monthNames.slice(
        0,
        currentMonth + 1
    );

    const chartDonationData = monthlyDonations.slice(
        0,
        currentMonth + 1
    );

    const chartUserData = monthlyUsers.slice(
        0,
        currentMonth + 1
    );
    ...
    const completedCampaigns = Math.round(
        (kpis?.campaigns?.total || 0) *
        (kpis?.campaigns?.performance?.completionRate || 0)
    );
                        ...
                        (range) => (
                            <button
                                ...
                                className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-colors ${
                                    timeRange === range
                                        ? 'bg-white dark:bg-brand-dark-200 shadow'
                        ...
                        {(
                            ...
                            (activity: Activity) => (
                                ...
                                >
                                    ...
                                    <div className="p-3 bg-gray-100 dark:bg-brand-dark rounded-full">
                    ...
                    <div className="bg-white dark:bg-brand-dark-200 p-6 rounded-lg shadow-lg">
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold">
                                    {systemHealth?.cpu?.loadAverage?.[0]?.toFixed(
                                        2
                                    ) || 'N/A'}
                                </span>
                            </div>
