...
export const organizationAPI = {
  ...
  getByUsername: async (username: string) => {
    ...
    if (user.role === 'ngo') {
      ...
      campaigns = publicCampaigns.filter(
        c => c.organizerId === user._id
      );
    }
...
export const taskAPI = {
  ...
  markTaskComplete: (id: string) =>
    request(`/user/tasks/${id}/complete`, {
      method: 'PATCH'
    }),
  ...
  getCalendarView: (params: {
    ...
    request(
      `/user/tasks/calendar/view?year=${params.year}&month=${params.month}`
    ),

  getTodaysTasks: () =>
    request('/user/tasks/today/list'),

  getUpcomingTasks: () =>
    request('/user/tasks/upcoming/list')
};
...
export const ngoAPI = {
  ...
  updateCampaign: (id: string, data: any) =>
    request(`/ngo/campaigns/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    }),

  getCompanies: () =>
    request('/ngo/companies'),

  getCompany: (id: string) =>
    request(`/ngo/companies/${id}`),

  getDonations: () =>
    request('/ngo/donations'),

  getUsers: () =>
    request('/ngo/users'),

  getVolunteering: () =>
    request('/ngo/volunteering'),

  getReports: () =>
    request('/ngo/reports'),

  getSettings: () =>
    request('/ngo/settings'),
...
export const companyAPI = {
  getDashboard: () =>
    request('/company/dashboard'),

  getDonationHistory: (filters: any) =>
    request('/company/donations', {
      method: 'POST',
...
export const donorAPI = {
  getDashboard: () =>
    request('/donor/dashboard'),

  getDonationHistory: (filters: any) =>
    request('/donor/donations', {
      method: 'POST',
      body: JSON.stringify(filters)
    }),
};
...
export const adminAPI = {
  ...
  getUserById: async (
    ...
    try {
      ...
      if (!userProfile?.user?._id) {
        ...
        console.error(
          "User profile structure not found in response for ID:",
          userId,
          response
        );
  ...
  generateNgoShareLink: (
    profileId: string
  ) =>
    request(
      `/admin/ngos/${profileId}/share`,
      {
        method: 'POST'
      }
    ),
  ...
  generateCompanyShareLink: (
    ...
    request(
      `/admin/companies/${profileId}/share`,
      {
        method: 'POST'
      }
    ),
  ...
  getShareablePageDesign: async (
    shareId: string
  ) => {

    const response =
      await request(
        `/admin/share/${shareId}/customize`
      );
  ...
  // ⭐ FIXED CAMPAIGN APPROVAL
  approveCampaign: (
    campaignId: string,
    status: 'approved' | 'rejected'
  ) =>
    adminAPI.updateCampaign(
      campaignId,
      {
        approvalStatus: status,
  ...
  generateCampaignShareLink: (
    campaignId: string
  ) =>
    request(
      `/admin/campaigns/${campaignId}/share`,
      {
        method: 'POST'
      }
    ),
  ...
  deleteCampaignFile: (
    ...
    return request(
      `/admin/campaigns/${campaignId}/file`,
      {
        method: 'DELETE',
        body: JSON.stringify({
  ...
  uploadCampaignImages: async (
    ...
    return request(
      `/admin/campaigns/${campaignId}/images`,
      {
        method: 'POST',
        body: formData,
      }
  ...
  uploadCampaignDocuments: async (
    ...
    return request(
      `/admin/campaigns/${campaignId}/documents`,
      {
        method: 'POST',
        body: formData,
      }
  ...
  uploadCampaignProofs: async (
    ...
    return request(
      `/admin/campaigns/${campaignId}/proof`,
      {
        method: 'POST',
        body: formData,
      }
  ...
  reportsAPI: {
    ...
    getUsersReport:
      async (filters: any) => {
        ...
        return request(
          `/admin/reports/users?${queryParams.toString()}`
        );
      },
    ...
    getCampaignsReport:
      async (filters: any) => {
        ...
        return request(
          `/admin/reports/campaigns?${queryParams.toString()}`
        );
      },
    ...
    getDonationsReport:
      async (filters: any) => {
        ...
        return request(
          `/admin/reports/donations?${queryParams.toString()}`
        );
      },
    ...
    getFinancialReport:
      async (filters: any) => {
        ...
        return request(
          `/admin/reports/financial?${queryParams.toString()}`
        );
      },
    ...
    exportReport:
      async (
        ...
        const url =
          `${API_BASE}/admin/reports/${reportType}?${queryParams.toString()}`;
        ...
        a.download =
          `${reportType}-report-${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
...
export const getSharedProfile = async (
  ...
  try {
    ...
    if (
      ...
      console.error(
        "Shared profile data structure not found for share ID:",
        shareId,
        response
      );
    ...
    console.error(
      `Error fetching shared profile with ID ${shareId}:`,
      error
    );
...
export const getSharedCampaign = async (
  ...
  try {
    ...
    if (
      ...
      console.error(
        "Shared campaign data structure not found in response for share ID:",
        shareId
      );
    ...
    console.error(
      `Error fetching shared campaign with ID ${shareId}:`,
      shareId,
      error
    );
...
export const getShareablePageDesign =
  adminAPI.getShareablePageDesign;

export const updateShareablePageDesign =
  adminAPI.updateShareablePageDesign;
